...
var Component = React.createClass({
    ...
    propTypes: {
        ...
        skipCallback: React.PropTypes.func,
    ...
    componentDidUpdate: function (prevProps, prevState) {
        ...
        if ((state.tooltip || (state.play && props.steps[state.index])) && (state.xPos < 0 || (prevProps.steps.length && props.steps.length && (prevProps.steps[0] !== props.steps[0])))) {
            this._calcPlacement();
        }
    ...
    finishCurrentTooltip: function () {
      this._onClickTooltip({ target: { dataset: { type: 'close' } } });
    },
    ...
    parseSteps: function (steps) {
        ...
        tmpSteps.forEach(function (s) {
            if (s.selector && s.selector.dataset && s.selector.dataset.reactid) {
                s.selector = '[data-reactid="' + s.selector.dataset.reactid + '"]';
            }

            el = s.selector ? document.querySelector(s.selector) : s.targetRect;
            s.position = s.position || 'top';
            if (!s.targetRect) {
              s.targetRect = el.getBoundingClientRect();
              s.targetRect.offsetParent = 1;
    ...
    _calcPlacement: function () {
        ...
        if (step) {
            ...
            target = step.selector ? document.querySelector(step.selector).getBoundingClientRect() : step.targetRect;
    ...
    _createComponent: function (update) {
        var state       = this.state,
            ...
            buttons     = {
                primary: props.showCloseButton ? props.locale.close : ''
            },
            ...
            component,
            targetRect  = currentStep && currentStep.targetRect;
        ...
        if (target || targetRect) {
            if (state.showTooltip || state.tooltip) {
                if (!state.tooltip) {
                    if ((props.type === 'continuous' || props.type === 'guided') && props.showCloseButton) {
